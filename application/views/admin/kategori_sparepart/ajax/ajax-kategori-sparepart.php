...
$(document).ready(function(){
    tableData = $('#konten').DataTable({
        ...
        "ajax": "<?php echo base_url('view-kategori-sparepart'); ?>",
        "columns" : [  
            {"data" :"id"},
            {"data" :"kategori"},
            {"data" : null,
                 "render":function(data){
                    return ' <button  style="margin:2px" class="btn btn-danger " onclick=deleteKategoriSparePart("'+data.id+'");><i class="ion-ios-trash"></i></button> ';
                }
            },
        ]
    } 
...
var ins = $('#form-add-new').on('submit', function(e){
      ...
      $.ajax({
        url: "<?php echo base_url('save-kategori-sparepart'); ?>",
        method: 'post',
...
function deleteKategoriSparePart(id){
    swal({
        title: "Peringatan",
        icon: "warning",
        ...
        if (ok) {
            $.ajax({
                url : "<?php echo base_url('delete-kategori-sparepart'); ?>",
                type: "POST",
                dataType: "JSON",
                data: {
                    id : id ,
