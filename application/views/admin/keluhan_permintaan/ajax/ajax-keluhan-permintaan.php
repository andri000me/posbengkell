...
$(document).ready(function(){
    tableData = $('#konten').DataTable({
        ...
        "ajax": "<?php echo base_url('view-keluhan-permintaan/'); ?>" + $("#id_penerimaan").val(),
        "columns" : [  
            {"data" :"id"},
            {"data" :"kategori"},
            {"data" :"keterangan"},            
            {"data" : null,
                 "render":function(data){
                    return '<a style="margin:2px" href="<?php echo base_url('edit-keluhan-permintaan/'); ?>'+ $("#id_penerimaan").val()  +'/'+  +data.id+'" class="btn btn-teal"><i class="ion-ios-create"></i></a> '+
                    		' <button  style="margin:2px" class="btn btn-danger " onclick=deleteKeluhan("'+data.id+'");><i class="ion-ios-trash"></i></button> ';
                }
            },
        ]
    } 
...
function deleteKeluhan(id){
    swal({
        ...
        if (ok) {
            $.ajax({
                url : "<?php echo base_url('delete-keluhan-permintaan'); ?>",
                type: "POST",
                dataType: "JSON",
                data: {
                    id : id ,
